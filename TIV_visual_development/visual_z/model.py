...
class InputAdapter(nn.Module):
    def __init__(self,information_mode='camera_map',extra_dim=6):
        super().__init__()
    ...
    def forward(self,obs,z):
        ...
        if self.information_mode=='vision_memory':   # v4：legacy 13 维已全部来自视觉记忆（无地图、无真值），不再屏蔽；附加 6 维记忆元信息
            extra=obs.extra.float() if obs.extra is not None else torch.zeros(o.shape[0],self.extra_dim,device=o.device)   # 离线池样本无记忆元信息 → 0
            if extra.shape[1]!=self.extra_dim:raise ValueError(f'extra 维数 {extra.shape[1]} 与配置 extra_dim={self.extra_dim} 不符')
