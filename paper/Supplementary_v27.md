...
## A. VEHICLE MODEL AND STRUCTURED OBSERVATIONS

### A.1. Traction and regenerative power

The longitudinal parameters follow the 2019 Nissan Leaf Plus benchmarking report [S4]: test mass $m=1928$ kg and coast-down road load
...
**Implemented efficiency model.** The loss function used to construct the efficiency map is

$$P_{\rm loss}(F,v)=P_0+k_{\rm Fe}v+k_{\rm Cu}F^2. \tag{A3}$$

Here $P_0=300$ W is an assumed standby term; $k_{\rm Fe}\approx32.4$ W·s/m and $k_{\rm Cu}\approx1.07\times10^{-4}$ W/N² are solved from two specified operating anchors: 96% at 4000 N and 15 m/s, and 90% at 421 N and 22.22 m/s. The 96% level follows [S4]; the selected operating coordinates, standby term, and cruise anchor define the engineering calibration.
...
### A.3. Structured-input normalization
...
## B. VISUAL PERCEPTION, MEMORY, AND LEARNING INTERFACE

### B.1. Information flow and training protocol
...
All twelve checkpoints use the original 87-dimensional policy input, including six extra memory features. The optional extra_dim=10 interface is not used by these weights. It exposes additional latch/commitment/observation fields but does not by itself establish a Markov representation: voting, consistency and release history can still affect future memory transitions. The evaluated logs do not contain its additional observed/observed_conf/committed fields. Its availability does not change the input used for the reported experiments.

### B.3. Visual-supervision objective

The delivered `perception_loss` uses six terms, with the outer learning coefficient $\lambda_v=1$ in the archived configurations:
...
Twenty-six software tests check registered Gymnasium interfaces, action parsing and the command mapping, deterministic seeded behavior, snapshot recovery, simultaneous instance isolation, command holding, reward/energy accumulation, terminal versus truncated outcomes, supported sampled release bounds, pinned rule-profile identities, and the candidate memory update. The finite API and smoke steps are software verification, distinct from the paper's frozen-policy driving experiments. CPU installation and command-line smoke checks require no Torch or CUDA import. SOFTWARE_VERIFICATION.json records tested distributions and commands. The companion provides a concrete reuse interface while leaving trained-policy quality, broader scenarios, calibrated error bounds, and real-road transfer unclaimed.

## D. RELEASE-AWARE EXECUTION AND TARGET REVISION
...
**Specified backup recursion.** Initialize $d=0$ at the supplied $(v,a)$ and reject $v-W_\delta((-a)_+)<-10^{-8}$. For at most 100 steps, accept a stopping terminal state when $v\le10^{-9}$ and $|a|<10^{-9}$; for $v_T>0$, accept $v\le v_T+10^{-9}$ with $a\le0$. Otherwise choose
...
**Candidate search and wrappers.** The base lower bound is $\max\{-3.5,a-q,-\bar b(v)\}$ and its upper bound is $\min\{2.6,a+q,\bar b((v_{\max}-v)_+)\}$ when a speed cap is supplied. A lower bound above the upper by more than $10^{-9}$ is infeasible. For each target, failure of the lower candidate's next-step distance check with $10^{-7}$ m tolerance returns `stop_distance_infeasible`. A tolerance-only pass retains just the lower endpoint. Otherwise a failing upper endpoint is reduced by 32 bisections, retaining the passing side. These are the delivered interval-search rules; no assertion that they enumerate every feasible action is needed.
...
## E. PREPARATION REFERENCE AND CLOSED-FORM SPECIAL CASE
...
**Table S1 — Complete half-second LP scans.** All columns refer to sampled grid points in seconds. The next point after each last feasible delay is infeasible; no entry is censored by the scan. The model excludes noisy range estimates, the deployed comfort envelope, and emergency projection.

| Initial speed, m/s | Jerk | Latest zero-penalty delay | Latest feasible delay | First infeasible delay |
|---|---:|---:|---:|---:|
| 22.2 | 2 | 5.0 | 8.0 | 8.5 |
| 22.2 | 3 | 5.0 | 8.5 | 9.0 |
| 22.2 | 4 | 5.5 | 9.0 | 9.5 |
...
**Proposition 1 — Optimal common preparation in the three-node family.** Scene-specific feasible maneuvers exist exactly when $\rho\ge1/2$; common preparation exists exactly when $\rho\ge2/3$. For $\rho\ge2/3$, the minimum expected complete-trip times satisfy
...
### E.5. Ideal energy-time consequence
...
### E.6. Necessary and excess preparation within the same model
...
![Closed-form preparation special case](figs/fig2_preparation.png)

**Fig. S1.** Closed-form maneuver in Proposition 1. The feasible stop/pass preparation intervals, normalized complete-trip information penalty, and passing-branch preparation profiles belong to the declared continuous three-node family. They are separate from the sampled LP and observed vehicle trajectories.

## F. COMFORT AND EXECUTOR TIMING EVALUATIONS

### F.1. Training protocol and soft-regularizer trade-off
...
### F.2. Executor configurations and information access
...
**Table S2 — Executor configurations on the development conditions.**

| configuration | completed / 18 | observed issue | interpretation |
|---|---|---|---|
...
### F.3. All paired comfort conditions
...
**Table S3 — Complete paired development effects.**

| actor | condition | $I_j$ reduction % | time change, s | energy change % |
|---|---|---|---|---|
| A | 0 | 22.22 | +0.5 | -0.89 |
| A | 1 | 27.90 | +0.0 | -1.37 |
| A | 2 | 28.48 | +0.5 | -0.27 |
...
### F.4. Acceleration diagnostics and endpoint conventions
...
**Table S4 — Supplementary acceleration diagnostics.**

| actor / executor | mean acceleration RMS, m/s$^2$ | largest braking magnitude, m/s$^2$ | mean time below $-2$ m/s$^2$, s |
|---|---|---|---|
...
The benefit also survives the original arrival cut: A's $I_j$ changes from **77.861 to 61.879**, a **20.53%** reduction; B's changes from **49.083 to 32.205**, a **34.39%** reduction. The larger settled-tail percentages use a different, explicitly matched denominator and must not be mixed with these values.
...
### F.6. Timing conditions and metric definitions
...
### F.7. Every paired timing condition

**Table S5 — Signal-window squared jerk and completion.** Values are in m$^2$/s$^5$. The color-only value in condition 7 is an incomplete window and must not be treated as a matched completed-window cost. The last column compares complete raw trajectories, not merely rounded metrics.

| condition | current color | color + timing | settled: color / timing | trajectory relation |
|---|---|---|---|---|
...
### G.2. Every seed and arm

**Table S6 — All visual seed summaries, with the JH terminal convention identified.** Mean R includes all nine attempts. I_j, time, and energy condition on each row's own completions and must not be subtracted as matched effects. JH rows retain the terminal override; F/S/J rows use current no-override records. Units are as in main Table III.

| Seed | Mode | Completed n / 9 | Mean R, all 9 | Mean I_j, completed | Time, completed | Energy, completed |
|---|---|---|---|---|---|---|
...
### G.3. Common-success contrasts

**Table S7 — Matched-protocol common-success contrasts relative to F.** Every row uses the same condition IDs and terminal convention on both sides; R is conditional on that intersection. The JH outcomes remain in Table S6 and are not used for cross-version contrasts.

| Contrast | Seed | Condition IDs | n | Delta I_j | Delta time | Delta energy | Delta R |
|---|---|---|---|---|---|---|---|
...
Signal phase is logged at substep start t0. The recorded crossing_speed field is v0 of that substep; exact crossing speed requires within-step kinematics.

Cue-removal probes change both the frozen perception path and the policy representation, supporting system dependence on visual cues without isolating Z. The constant-command v4r reference completes nine conditions without signal or curve-speed violations, with I_j=100.954 and peak jerk=12.2 m/s^3. It has rounded records and incomplete evaluation identity, so it remains descriptive.
...
**Table S8 — Every affected S episode.** Seeds 0/1/2 denote the three reported policies. Values are at the first override's pre-step state; positions and velocities are rounded. The signal-terminal label here is a contextual category, distinct from the below-5 m/s P-B threshold.

| Seed / condition | First adopted color | Position, m | Speed, m/s | First-event context |
|---|---|---:|---:|---|
...
## H. PREPARATION BRANCHES AND TARGET-UPDATE DIAGNOSIS

### H.1. Frozen-state design and output gating
...
### H.2. Information histories before adoption
...
**Table S9 — Residual scene distinctions in delayed constant-command pairs.** Differences are checked at equal absolute times while both phase outputs remain hidden, with numerical tolerance $10^{-8}$. A pair is counted once if any such difference occurs.

| Speed / preparation | Pairs | Track-onset differences | Range-history differences | Applied-acceleration differences |
|---|---:|---:|---:|---:|
...
For example, at 22.2 m/s, jerk 2, conservative preparation, and target adoption 67 m, the stopping track is established 3.0 s after freezing, before the passing track. At 3.5 s, both phases remain hidden, yet the applied accelerations are $-1$ and $0$ m/s$^2$. The branch identifiers are `99bce1c256d1` and `ea5b8d6e5ad9`. Their physical states agree before the first track difference.
...
### H.3. Completion, recovery, and fixed-endpoint cost
...
### H.5. Record timing and event selection
...
The source base `curve_layer.py` has SHA-256 `3c20801fa67df955dc85aa314a620d5cfd8ae6320a0cb64eb66fe034eb5764a0` in both P-C and P-V. The same routine, parameterized by the branch jerk, computes all backups below. This establishes base identity, not equal wrappers, information access, or policy states.

### H.6. Target margins at adoption
...
**Table S12 — Adoption margins and subsequent outcomes.** Distances are metres. P-B ranges use all stopping branches. P-V target values use the 18 initially nonpassable episodes; the other nine initially green episodes have no stopping target at adoption. P-V episode overrides are not assigned to that initial event.

| Cohort | N | True-line margin | Estimated-line margin | Actual stopping-target margin | Outcome count |
|---|---:|---:|---:|---|---|
...
### H.7. Actual-delay LP references and residuals
...
**Table S13 — Passing cost and LP-reference residuals.** All ranges are seconds. Floor residuals exist for all 102 rows. Interpolation is secondary and has one unavailable value because its upper program is infeasible.

| Initial speed | Rule | N | Observed difference | Floor residual | Interpolated residual | Finite interpolation |
|---|---|---:|---:|---:|---:|---:|
...
**Table S14 — Two bracketing cases.** Detailed branch values are retained in lp_reference_decomposition.csv. The second row has an available floor reference, not a missing or zero cost.

| Case | Actual delay, s | Observed difference, s | Lower LP reference, s | Upper LP reference, s | Consequence |
|---|---:|---:|---:|---|---|
...
### H.8. State-fixed target substitutions
...
**Algorithm 2 — Offline target-trigger diagnosis.**

1. Find adoption and the first strictly subsequent event within the declared approach window.
2. Retrieve the physical state and memory preceding its triggering action.
3. Evaluate $D_\kappa$, the true-line, estimated-line, and actual buffered-target margins.
4. With that state, branch jerk, backup routine, and free-road cap fixed, call the single-target interval routine for each of the three target distances.
5. Record the returned reason and interval, then classify the event by speed and location. Evaluate range innovations only before the triggering action.

**Table S15 — State-fixed single-target substitutions.** Each row uses the same 195 event-preceding states. The caps are the corresponding 22.2 or 16 m/s free-road speeds; protocol envelopes, other targets, and future observations are not re-simulated. Truth is an offline reference only.

| Target supplied to interval routine | Nonempty / 195 | Returned outcome |
|---|---:|---|
...
**Algorithm 3 — Experimental guarded memory update (record-replay evaluation).**

1. Propagate memory using the displacement measured over the same physical interval as vehicle integration. In this simulator replay, use recorded travel; no target or lamp truth is supplied to memory.
2. Evaluate the archived detection thresholds, fusion, and near-line rules to obtain the candidate signal range.
3. Reject only an inward fusion crossing of the 15 m range-freeze threshold as defined above; preserve the propagated range in that case.
4. Run the remaining archived phase, freshness, and commitment logic and record the update decision.
5. Carry this complete memory to the next logged observation; evaluate the unchanged backup at selected recorded physical states. Keep local one-update substitutions separate from recursive replay.

**Table S17 — Selected P-V terminal states under fixed-record replay.** Entries are buffered backup margins in metres at the original physical state and previous applied acceleration. M1/M2 are recursively replayed from reset. They are not new vehicle trajectories or reductions in override counts.

| Seed / condition | Original | M1: consistent displacement | M2: displacement and guard |
|---|---:|---:|---:|
...
## I. STRUCTURED 20 KM REFERENCE STUDY

### I.1. Numerical references

The DP uses 400 route cells of 50 m, 33 boundary-speed levels, and 1800 phase bins over the 90 s signal cycle. Cell crossing time is $2\Delta x/(v_j+v_k)$. It caps deceleration at 2.0 m/s² and evaluates costs at a fixed input pack state. These approximations differ from the simulator's evolving SOC and constrained tracking dynamics.
...
### I.2. Programmed driver policies
...
| driver | $d_{\text{see}}$ (m) | $a_b$ (m/s²) | $a_{\text{acc}}$ (m/s²) | behaviour |
|---|---|---|---|---|
...
### I.3. Conditions and training starts

At each training reset, the learner samples one of three pack regimes: SOC 0.85 at +15 °C, SOC 0.90 at −10 °C, or SOC 0.95 at −10 °C. It does not sample SOC and temperature continuously over a broad range. Signal offsets are independently uniform during training. Evaluation crosses the three regimes with shared offsets 0, 22.5, 45, and 67.5 s, applied to both signals.
...
### I.4. Multistep targets and optimization settings

An actor generates the acceleration command; two critics and delayed actor updates follow TD3 [R-TD3], with a 20-decision multistep target:
...
### I.5. Completed-trip reference accounting
...
**Table S18 — Reference-relative decomposition.** Denominator $n$ counts completed reporting conditions; seed 5's full score is separately retained in Table S19.

| controller | pacing | execution | penalty | total | execution % | n |
|---|---|---|---|---|---|---|
...
### I.6. All seeds, failures, and checkpoint selection

**Table S19 — Reporting scores and physical summaries.** All selected scores average six conditions, including failures. DP rows are tracked references; energy is Wh and each seed's late entry gives mean (sample SD) across eight checkpoints. The final row instead gives the mean and sample SD across the eight seed summaries.

| controller | return | battery | time s | friction | arrivals / 6 | late return (SD) |
|---|---|---|---|---|---|---|
| ref | -168.18 | 2204.9 | 1110.0 | 5.5 | 6 | — |
| info | -170.46 | 2173.5 | 1152.8 | 20.4 | 6 | — |
| attentive | -177.47 | 2732.0 | 989.0 | 157.3 | 6 | — |
| normal | -179.42 | 2801.7 | 982.0 | 210.9 | 6 | — |
| distracted | -181.01 | 2854.3 | 978.0 | 253.4 | 6 | — |
| rule 18.5 | -177.29 | 2388.6 | 1141.2 | 191.6 | 6 | — |
...
| across seeds | -177.37 (3.31) | — | — | — | — | -183.15 (5.75) |
...
The speed-only 18.5 m/s rule uses the same execution envelope and decision interval: return **−177.29**, **6/6** arrivals and no red-light or overspeed violations. Under the last-eight-checkpoint mean, **2 of 8** seeds outperform this rule, compared with **5 of 8** under validation-selected checkpoints. Seeds 4, 5, and 7 score below it. Thus part of the gain over the driver family is available without richer observation; channel use requires direct measurement.

Across seeds, the selected mean is **-177.37** with sample SD **3.31**; the late mean is **-183.15** with between-seed sample SD **5.75**. Relative to the rule, these mean differences are **-0.08** and **-5.86**. These summaries do not establish an average advantage; the best seed is an observed case, not typical performance.

Seed 5's selected return includes its unfinished condition. Its all-six-condition gap to the condition-matched sampled reference minima is **14.68**; Table S18's 12.82 conditions on five completions. Failed distance cannot be interpreted as a completed low-energy trip.

Across saved evaluations, **45 of 1920** trips violate a red light, with every seed contributing. None occurs at the eight selected checkpoints on the twelve-condition grid. The largest logged jerk is 9.78 m/s³, above the nominal target because of emergency overrides; five selected seeds have nonzero overspeed. These are measured limits of the structured-input control stack.

## J. REPRODUCTION AND DATA PROVENANCE

### J.1. Current calculations and evidence boundaries
...
From the package root, check the manifest before rebuilding. Run `verification/framework/audit_offline.py` for adoption and passing-reference rows, `verification/diagnostics/audit_events.py` for target substitutions, visual localization and complete LP scans, and `verification/target_memory/audit_memory.py` for trigger-aligned accounting and recursive memory replay. `verification/check_v27.py` checks retained numbers, equation and table labels, semantic section references, and scientific scope. `latex/build_pdfs.py` builds both PDFs from their Markdown sources.

The complete P-V replay input is `verification/target_memory/pv_full_layer_logs.json.gz`; its 27 episodes contain 12,081 recorded timestamps. Compact P-B excerpts suffice for the published target-update calculations but do not contain complete detection streams. Full 864-branch trajectories, visual weights and image pools, and long-route training archives remain external source archives identified in the provenance files. Running network inference or repeating vehicle evaluations requires those assets; a local paper build and its record calculators do not.
