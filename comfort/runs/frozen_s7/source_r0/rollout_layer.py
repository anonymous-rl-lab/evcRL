...
def digest():
    return hashlib.sha256(''.join((ROOT/p).read_text() for p in ['code/curve_layer.py','code/rollout_layer.py','protocol/PROBE_v2.md']).encode()).hexdigest()
...
def main():
    ...
    codehash=digest();out=ROOT/'runs/frozen_s7';out.mkdir(parents=True,exist_ok=True)
    f=out/'progress.json'
