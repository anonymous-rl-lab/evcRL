...
class VisionMemory:
    def __init__(self, det_thr=0.5, hold_s=3.0, curve_len_max=1000., release_grace_m=5., init_votes=2, vote_window=3, consistency_m=40., expire_s=4.0, light_expire_s=10.0,
                 ...
                 stationary_expire_s=10.0, *, route_length=4000., propagation="legacy", guard=False):
        ...
        if propagation not in ("legacy", "consistent"): raise ValueError(propagation)
        self.route_length=float(route_length); self.propagation=propagation; self.guard=bool(guard)
    ...
    # ---------------------------------------------------------------- 观测融合
    def update(self, dets, color_probs, v, dt, *, odom_ds=None):
        ...
        if self.propagation == "consistent":
            if odom_ds is None or not math.isfinite(odom_ds) or odom_ds < 0: raise ValueError("consistent propagation requires finite nonnegative odom_ds")
            ds = float(odom_ds)
        self.update_diagnostic = dict(odom_ds=ds, range_guard=False)
        ...
        if hit and (self.sig['seen'] or (ok and not near_end)) and lt['dist_m'] <= LIGHT_RANGE + LIGHT_AHEAD and color_probs is not None:
            ...
            if self._accept('traffic_light', self.sig['d_line'] if self.sig['seen'] else None, new_d) or (self.mismatch['traffic_light'] >= 3 and lt['score'] >= self.thr('traffic_light') and ok):
                ...
                predicted = self.sig["d_line"]
                candidate = fused
                guarded = bool(self.guard and self.sig["seen"] and predicted is not None and predicted >= self.dist_freeze_m and candidate < self.dist_freeze_m and candidate < predicted)
                if guarded: fused = predicted
                self.update_diagnostic.update(predicted=predicted, candidate=candidate, accepted=fused, range_guard=guarded, raw_line=new_d)
    ...
    def executor_targets(self, x):
        ...
        if self.curve['announced'] and not self.curve['active'] and self.curve['d_est'] is not None:
            out.append((x + max(self._margin(self.curve['d_est']) - 15.0, 0.), V_CURVE, 2.0))
    ...
    def legacy(self, v, a, soc, T, t_end, t, t_budget):
        ...
        d_end = self.end['d_est'] if self.end['d_est'] is not None else self.route_length
        dc = self.curve['d_est'] if (self.curve['announced'] and not self.curve['active'] and self.curve['d_est'] is not None) else 400.0
        vc = V_CURVE if self.curve['announced'] or self.curve['active'] else V_FREE
        ds = self.sig['d_line'] if (self.sig['seen'] and self.sig['d_line'] is not None) else 1000.0
        phase = {'green': 1., 'red': 0., 'yellow': 0., 'off': 0.}.get(self.sig['phase'], -1.) if self.sig['seen'] else -1.
        return np.array([v / V_FREE, a / 3.5, max(d_end, 0.) / self.route_length, self.v_limit() / V_FREE, min(max(dc, 0.), 400.0) / 400.0, vc / V_FREE,
                         min(max(ds, 0.), 1000.0) / 1000.0, phase, 1.0, (soc - .5) / .5, (T - 283.15) / 30., (t_end - t) / t_budget, t / t_budget], np.float32)
    ...
    def set_state(self, state):
        if state.get("propagation") != self.propagation or state.get("guard") != self.guard: raise ValueError("memory profile mismatch")
